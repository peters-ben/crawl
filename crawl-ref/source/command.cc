...
class settings_popup : public formatted_scroller
{
public:
	settings_popup(int key) : formatted_scroller(FS_PREWRAPPED_TEXT) {
		set_tag("help");
		process_key(key);
	};
...
void show_settings(int section, string highlight_string) {
	...
	int index = 0;
	std::string line;
	std::ifstream in_file ("/u/zon-d2/ugrad/bspe227/crawl/crawl-ref/settings/settings_command_keys.txt");
	if(!in_file)
